...
SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID, LOCAL_NAME FROM DEPARTMENT NATURAL JOIN LOCATION;

-- 3. 보너스를 받는 사원들의 사번, 사원명, 보너스, 부서명 조회
SELECT EMP_ID, EMP_NAME, BONUS, DEPT_TITLE FROM EMPLOYEE NATURAL JOIN DEPARTMENT WHERE ;

-- 4. 부서가 총무부가 아닌 사원들의 사원명, 급여 조회
SELECT EMP_NAME, SALARY, DEPT_TITLE FROM EMPLOYEE NATURAL JOIN DEPARTMENT WHERE DEPT_TITLE != '총무부';
...
FROM EMPLOYEE E, DEPARTMENT D, LOCATION L, JOB J, NATIONAL N, SAL_GRADE S
...
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE) -- 직급
JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID) -- 부서
JOIN LOCATION L ON(D.LOCATION_ID = L.LOCAL_CODE) -- 근무지역
...
WHERE E.DEPT_CODE = D.DEPT_ID
AND E.JOB_CODE = J.JOB_CODE
AND EMP_NAME LIKE '전%' AND SUBSTR(EMP_NO, 8, 1) = 2 AND EMP_NO LIKE '7%';
...
--    급여 등급 S1, S2 --> '고급'   S3, S4  --> '중급'  S5, S6 --> '초급'
...
WHERE E.JOB_CODE = J.JOB_CODE -- 직급
AND E.DEPT_CODE = D.DEPT_ID -- 부서코드
