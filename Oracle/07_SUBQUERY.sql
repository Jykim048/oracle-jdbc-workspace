...
/*
   ...
   - 서버쿼리의 종류에 따라 서브쿼리 앞에 붙는 연산자가 달라짐
*/
/*
   1. 단일행 서브쿼리(SINGLE ROW SUBQUARY)
...
-- 전지연 사원이 속해있는 부서원들의 사번, 사원명, 전화번호, 직급명, 부서명, 입사일 조회 (단, 전지연 사원은 제외) 직급명부서명
...
-- 위의 급여를 받는 사원들 조회
SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE SALARY IN(2890000, 3660000, 8000000, 3760000, 3900000, 2490000, 2550000);
...
FROM EMPLOYEE E JOIN JOB J USING(JOB_CODE) WHERE J.JOB_NAME = '과장';

/*
   비교대상 > ALL(서브쿼리) : 여러개의 "모든" 결과값들 보다 클 경우
   비교대상 < ALL(서브쿼리) : 여러개의 "모든" 결과값들 보다 작을 경우
*/
-- 과장 직급임에도 차장 직급의 최대 급여보다 더 많이 받는 직원들의 사번, 이름, 직급명, 급여 조회 (사원 -> 대리 -> 과장 -> 차장 -> 부장)

-- 차장 직급들의 급여 조회
-- >> 오라클 구문
SELECT SALARY FROM EMPLOYEE E, JOB J WHERE E.JOB_CODE = J.JOB_CODE AND JOB_NAME = '차장':
-- >> ANSI
SELECT SALARY FROM EMPLOYEE E, JOIN JOB J USING(JOB_CODE) WHERE JOB_NAME = '차장';
...
WHERE E.JOB_CODE = J.JOB_CODE
AND JOB_NAME = '과장'
AND SALARY > ALL(SELECT SALARY FROM EMPLOYEE E, JOB J WHERE E.JOB_CODE = J.JOB_CODE AND JOB_NAME = '차장');
...
SELECT ROWNUM, EMP_NAME, SALARY FROM (SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC)
WHERE ROWNUM <= 5;
