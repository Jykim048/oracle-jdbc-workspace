...
-- 사원의 사번, 사원명, 직급명, 성별(남/여), 근무년수를 조회할 수 있는 SELECT문을 뷰(VW_EMP_JOB)로 정의
...
SELECT 사원명, 직급명 FROM VW_EMP_JOB WHERE 성별 = '남';
...
-- 뷰를 통해서 UPDATE (JOB_CODE가 J8인 데이터를 JOB_NAME을 알바로 변경)
...
-- 2. 뷰에 포함되지 않은 컬럼 중에 베이스가 되는 컬럼이 NOT NULL 제약조건이 지정된 경우
CREATE OR REPLACE VIEW VW_JOB AS SELECT JOB_NAME FROM JOB;
...
-- UPDATE
UPDATE VW_JOB
SET JOB_NAME = '알바' WHERE JOB_NAME = '사원';
...
UPDATE VW_EMP_SAL SET 연봉 = 80000000 WHERE EMP_ID = 200; -- 에러 (산술연산으로 정의된 컬럼은 데이터 변경 불가능)
...
-- 6. JOIN을 이용해서 여러 테이블을 연결한 경우 사원들의 사번, 사원명, 부서명 조회 -> VW_JOINEMP
