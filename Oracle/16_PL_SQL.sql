...
--   END;
 ...
 /*
    1_2) 레퍼런스 타입 변수 선언 및 초기화
        (어떤 테이블의 어떤 컬럼의 데이터타입을 참고해서 그 타입으로 지정)
 ...
 */  
 ...
 -- 레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL를 선언하고 각 변수의 자료형은 EMPLOYEE 테이블의 
 -- EMP_ID, EMP_NAME, JOB_CODE, SALARY 컬럼과 DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형을 참조
 -- 사용자가 입력한 사번과 일치하는 사원을 조회한 후 결과를 출력
 
  DECLARE
  EID EMPLOYEE.EMP_ID%TYPE;
 ENAME EMPLOYEE.EMP_NAME%TYPE;
 JCODE EMPLOYEE.JOB_CODE%TYPE;
 DTITLE EMPLOYEE.DEPT_TITLE%TYPE;
 SAL EMPLOYEE.SALARY%TYPE;
 BEGIN
...
/*
   1-4) CASE 구문
       ...
       CASE 비교대상자
          ...
          END;
*/
-- 사번을 입력받은 후 사원의 모든 컬럼 데이터를 EMP에 대입
...
-- TOO_MANY_ROWS : SELECT한 결과가 여러 행인 경우 (한 행이 리턴되어야 하는
DECLARE
   EID EMPLOYEE.EMP_ID%TYPE;
