 /*
   ...
   - SQL(Structed Query Language)
     : 관계형 데이터베이스에서 데이터를 조회하거나 조작하기 위해 사용하는 표준 기본 언어
     
   - SQL 종류
    1. DDL (Data Definition Language) : 데이터 정의어
       - DBMS의 구조를 정의하거나 변경, 삭제하기 위해 사용하는 언어(CREATE 생성, ALTER 수정, DROP 삭제)
    2. DML (Data Manipulation Languagee) : 데이터 조작어
  ...
  */
  ...
  -- EMPLOYEE 테이블의 전체 사원들의 사번(EMP_ID), 이름(EMP_NAME), 급여만을 조회(SALARY)
...
SELECT EMP_ID 사번, EMP_NAME 직원명, SALARY 급여, '원' AS "단위(원)" FROM EMPLOYEE;
...
-- 부서코드가 'D1'이 아닌 사원들이 사번, 사원명, 부서코드 조회
SELECT EMP_ID, SALARY, DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE != 'D1';
...
/*
  LIKE
  - 비교하고자하는 컬럼값이 내가 제시한 늑정 패턴에 만족될 경우 조회
  ...
  비교대상컬럼 LIKE '__문자' => 비교대상컬럼값이 문자앞에 무조건 두글자가 올 경우 조회
...
-- 위의 사원들 이외의 사원들 조회
-- 논리부정연산자 : NOT
SELECT EMP_ID, EMP_NAME, PHONE, EMAIL FROM EMPLOYEE WHERE EMAIL NOT LIKE '___$_%' ESCAPE '$';
 ...
 */
 ...
 FROM EMPLOYEE WHERE DEPT_CODE NOT IN ('D6', 'D8', 'D5'); 
...
*/
 ...
 SELECT EMP_ID, EMP_NAME, SALARY, (SALARY + (BONUS * SALARY)) * 12 FROM EMPLOYEE
 ...
 /*
  ...
  ORDER BY 정렬하고자하는 컬럼값 [ASC|DESC] [NULLS FIRST|NULLS LAST];
   ...
   - NULLS LAST : 정렬하고자하는컬럼값이 NULL이 있을 경우 해당 데이터를 맨 뒤 배치 (생략시 ASC일때 기본값)
*/
