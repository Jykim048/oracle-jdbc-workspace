...
/*
   ...
   - LENGTHB(컬럼|'문자열값') : 해당 문자열값의 바이트 수 반환
   
   한글 한 글자 -> 3BYTE
   영문자, 숫자, 특수문자 한 글자 -> 1BYTE
*/
SELECT LENGTH('오라클'), LENGTHB('오라클'), LENGTH('ORACLE'), LENGTHB('ORACLE') FROM DUAL; -- 가상 테이블 
...
SELECT EMAIL, INSTR(EMAIL, '@'), INSTR(EMAIL, 'S', -1, 2) FROM EMPLOYEE WHERE EMAIL LIKE '%s%';

/*
   LPAD / RPAD
   - 문자열을 조회할 때 통일감있게 조회하고자 할 때 사용
   - LAPD/RPAD(STRING, 최종적으로 반환할 문자의 길이, [덧붙이고자 하는 문자])
...
/*
   ...
   - LTRIME / RTRIM(STRING, [제거하고자하는문자들])
...
/*
  ...
  - TRIM([LEANDING|TRAILING|BOTH] 제거하고자하는문자들 FROM STRING)  
*/
...
-- 여자사원들의 이름만 조회
SELECT EMP_NAME FROM EMPLOYEE WHERE SUBSTR(EMP_NO, 8, 1) IN (2, 4);

-- 남자사원들의 이름만 조회
SELECT EMP_NAME FROM EMPLOYEE WHERE SUBSTR(EMP_NO, 8, 1) IN (1, 3);
...
SELECT EMP_NAME, REPLACE(EMAIL, 'kh.or.kr', 'gmail.com') FROM EMPLOYEE;
 ...
 */
 ...
 -- 직원명, 입사일, 근무 개월 수 조회
 SELECT EMP_NAME, HIRE_DATE, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR(SALARY, 'L99,999,999'), TO_CHAR(12* SALARY, 'L99,999,999') as "연봉"
FROM EMPLOYEE
ORDER BY "연봉";
...
*/
-- 전체 사원의 총 급여 합
SELECT TO_CHAR(SUM(SALARY), 'FM999,999,999') FROM EMPLOYEE;
...
*/   
-- 전체 사원의 평균 급여 조회
SELECT TO_CHAR(ROUND(AVG(SALARY)), 'FM99,999,999') FROM EMPLOYEE;
...
/*
   ...
   COUNT(DISTINCT 컬럼) : 해당 컬럼값의 중복을 제거한 행의 개수 반환
*/
-- 전체 사원 수
